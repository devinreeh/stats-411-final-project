...
Conduct canonical correlation analysis using the package `CCA`. Split the data into two sets: one set for next day variables and another for technical indicators (e.g., RSI, CCI, moving averages).
...
```{r}
# Select columns
data_subset <- data[, c("rsi_14", "cci_14", "sma_50", "volume",
...
Find appropriate number of factors for factor analysis by looking at the eigenvalue
and scree plot.

```{r}
cor(data_subset)
ev <- eigen(cor(data_subset))
ev$values
scree(data_subset, pc = FALSE)
fa.parallel(data_subset, fa ="fa")
```

Use 3 factors according to parallel analysis.
...
# Conduct factor analysis
fa_result <- fa(data_subset,
                nfactors = num_factors,
                rotate = "varimax")

# Print factor loadings, scores, and summary
...
Use 2 factors for comparison, as simplified model.
...
# List respective columns
col_price <- c("next_day_close", "next_day_volume")
col_indicators <- c("rsi_14", "cci_14", "sma_50", "volume",
                    "ema_50", "sma_100", "ema_100", "bollinger",
                    "macd", "TrueRange", "atr_14")
